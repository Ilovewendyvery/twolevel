classdef TwoLevel_onehourold < handle
    ...
    properties
        ...
        %Dual variable
        tLambda;       %[lambda1,lambda2,lambda3] 
    ...
    methods
        function obj = TwoLevel_onehourold(NT) 
        ...
        function UpdatePowerDemand(obj,t) 
            for i=1:obj.MA.N
                b=obj.MA.D_it(i,t);
                [DAi, ~] = fminbnd(@(x) -obj.U_fun(x,b)+obj.tLambda(1)*x, 0, b);
                obj.tDA(i)=DAi;
            end
 
            for j=1:obj.MB.N
                b=obj.MB.D_it(j,t);
                [DBj, ~] = fminbnd(@(x) -obj.U_fun(x,b)+obj.tLambda(2)*x, 0, b);
                obj.tDB(j)=DBj;
            end 

            b=obj.MC.D_it(1,t);
            [obj.tDC, ~] = fminbnd(@(x) -obj.U_fun(x,b)+obj.tLambda(3)*x, 0, b);

            obj.tPowerDemand=[sum(obj.tDA),sum(obj.tDB),sum(obj.tDC)]; 
        end

        function UpdateLambda(obj,k)
            Lambda_old=obj.tLambda;
            obj.tLambda=obj.tLambda+obj.pp.*(obj.tPowerDemand-obj.tPowerSupply); 
            err1=norm(obj.tPowerDemand-obj.tPowerSupply);
            err2=norm(obj.tLambda-Lambda_old);
            for kk=1:3
                if obj.tLambda(kk) < 1
            ...
            disp([obj.tPowerDemand;obj.tPowerSupply;obj.tLambda])
            disp(obj.pp)
        ...
        function UpdatePowerSupply(obj,t)
            ...
            funA = @(x) obj.G_fun(x(1))-obj.tLambda(1)*(x(1)+obj.MA.PV(t)-x(2)-x(3)+x(4))...
            ...
            funB = @(x) obj.G_fun(x(1))-obj.tLambda(2)*(x(1)+obj.MB.PV(t)+x(2)-x(3)+x(4))...
            ...
            funC = @(x) obj.G_fun(x(1))-obj.tLambda(3)*(x(1)+obj.MC.PV(t)+x(2)+x(3)+x(4))...
        ...
        function draw(obj,t)
            ...
            ymax=max([obj.E_fun(x,obj.MA.SOC(t),-obj.MA.BC*(1-obj.MA.SOC(t))),obj.tLambda(1)*x]);
            ymin=min([obj.E_fun(x,obj.MA.SOC(t),-obj.MA.BC*(1-obj.MA.SOC(t))),obj.tLambda(1)*x]);
            plot(x,obj.tLambda(1)*x,'g-')
        ...
        function draw3(obj,t)
            ...
            ymax=max([obj.E_fun(x,obj.MC.SOC(t),20),obj.tLambda(3)*x]);
            ymin=min([obj.E_fun(x,obj.MC.SOC(t),20),obj.tLambda(3)*x]);
            plot(x,obj.tLambda(3)*x,'g-')
