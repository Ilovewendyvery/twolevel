classdef New < handle
    ...
    methods
        function obj = New()
            %     [a,b,c,p]=setdata();
            %     obj.A=a;obj.B=b;obj.C=c;obj.Pu=p;
            ...
            for i = 1:obj.n
                ...
                D(i, :) = base_load(user_type) * pattern + 0.2*randn(1, obj.T);
            ...
            %% 存储结果
            obj.X_opt = zeros(obj.n, obj.T);      % 最优用电量 [kW]
            obj.G_opt = zeros(1, obj.T);      % 最优电网供电 [kW]
            obj.B_opt = zeros(1, obj.T);      % 最优电池充放电 [kW]（正为放电，负为充电）
            obj.SOC_opt = zeros(1, obj.T);    % SOC轨迹
            obj.Lambda_opt = zeros(1, obj.T); % 最优对偶变量
        end
        function f=C_grid_func(obj,x)
            % 电网成本函数
            f=obj.a0*x^2 + obj.b0*x + obj.c0;
        end

        function run_inital(obj)
            %% 初始化SOC
            obj.total_cost_all = 0;
            obj.convergence_info = zeros(obj.T, 2); % 存储每个时段的收敛信息
        ...
        function  run(obj)
            ...
            [total_utility, total_grid_cost, total_battery_cost] = ...
                analyze_costs(obj.X_opt, obj.G_opt, obj.B_opt, obj.SOC_opt, obj.D, ...
        ...
        function x=argmin_x(obj,x,G,B,lambda, rho_t,t)
            % 并行更新每个用户的用电量
            for i = 1:obj.n
                % 用户i的本地优化问题
                z0=sum(x)-x(i)-(G+obj.PV(t)+B);
                fun = @(xi) user_local_optimization(xi, obj.D(i, t), lambda, z0, ...
                    obj.beta, obj.sigma, rho_t);

                % 边界约束
                lb = 0;
                ub = 1 * obj.D(i, t); % 最大不超过2.5倍基准需求
        ...
        function [G,B,z]=argmin_other(obj,x,lambda, rho_t,t,SOC_current)
            % 系统优化：最小化电网成本+电池成本+ADMM惩罚项
            [G_new, B_new, feasible, ~] = system_optimization(...
                obj.PV(t), SOC_current, ...
                lambda, sum(x), rho_t, obj.time_price(t), ...
                obj.a0, obj.b0, obj.B_max, obj.E_max);
        ...
        function [G,B,z]=argmin_other2(obj,x,lambda, rho_t,t,SOC_current)

            %bTab=0;bTac=0;Tbc_new=0;bTbc=0;

            tmu=[0,0,0];
            for k2=1:40
                [G_new, B_new, Tab_new, Tac_new] = system_optimizationA(...
                    obj.PV(t), SOC_current, ...
                    lambda, sum(x), rho_t, obj.time_price(t), ...
                    obj.a0, obj.b0, obj.B_max, obj.E_max,tmu);
                [G_newB, B_newB, bTab, Tbc_new] = system_optimizationB(...
                    obj.PV(t), SOC_current, ...
                    lambda, sum(x), rho_t, obj.time_price(t), ...
                    obj.a0, obj.b0, obj.B_max, obj.E_max,tmu);
                ...
                mu_old=tmu;
                primal_res=[Tab_new-bTab;Tac_new-bTac;Tbc_new-bTbc];
                tmu=tmu-1*primal_res;
        ...
        function hour(obj,t,SOC_current)
            ...
            for k = 1:obj.max_admm_iter
                ...
                %% 步骤1: 更新x（用户侧，可并行）
                x=argmin_x(obj,x,G,B,lambda, rho_t,t);



                %% 步骤2: 更新系统变量 (z, G, B)
                [G,B,z]=argmin_other2(obj,x,lambda, rho_t,t,SOC_current);
                ...
                for i = 1:obj.n
                    user_utility = user_utility + piecewise_log_utility(...
                        x(i), D_t(i), obj.beta, obj.sigma);
                end

                grid_cost_val = obj.C_grid_func(G) + time_price_t * G;
                battery_cost_val = battery_cost(B, SOC_current);

                cost_history(k) = -user_utility + grid_cost_val + ...
                    battery_cost_val;

                %% 收敛检查
                eps_primal = obj.abs_tol + obj.rel_tol * max(abs(sum(x)), abs(G+PV_t+B));
            ...
            % 记录收敛信息
            obj.convergence_info(t, 1) = iter_used;
            obj.convergence_info(t, 2) = primal_res_history(min(iter_used, obj.max_admm_iter));
            ...
            grid_cost_t = obj.C_grid_func(G) + time_price_t * G;
            battery_cost_t = battery_cost(B, SOC_current);
        ...
        function draw(obj)
            %% 可视化结果
            visualize_results(obj.X_opt, obj.G_opt, obj.B_opt, obj.PV, obj.SOC_opt, obj.D, obj.time_price, ...
